...
pub fn extract_from_tables(text: &str) -> Vec<String> {
    eprintln!("LSP: Extracting tables from FROM clause");
    ...
    if let Some(from_pos) = text.find("from(") {
        ...
        // Extract the content inside from()
        let mut paren_count = 0;
        let mut end_pos = None;
        
        for (i, ch) in after_from.chars().enumerate() {
            match ch {
                '(' => paren_count += 1,
                ')' => {
                    if paren_count == 0 {
        ...
        if let Some(end) = end_pos {
            let from_content = &after_from[..end];
            eprintln!("LSP: FROM content: '{}'", from_content);
...
/// Check if we're in a context where table/alias references would be appropriate
pub fn is_table_reference_context(text: &str, _before_cursor: &str) -> bool {
    eprintln!("LSP: Checking for table reference context");

    // Check if we're in a select, where, join, etc.
    let contexts = [
        ".select(", ".where(", ".innerJoin(", ".leftJoin(",
    ...
    // Check if any of these contexts are open (not closed)
    for context in &contexts {
        if let Some(pos) = text.rfind(context) {
            ...
            let mut paren_count = 1; // Start with 1 since we're inside the opening (
            ...
            // If paren_count > 0, we're still in this context
            if paren_count > 0 {
                eprintln!("LSP: Found table reference context: {}", context);
                return true;
            }
        }
    }
