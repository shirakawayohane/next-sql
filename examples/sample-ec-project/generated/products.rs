...
pub async fn find_product_by_id(
    ...
    let rows = client.query(
        "SELECT products.*, categories.name FROM products INNER JOIN categories ON categories.id = products.category_id WHERE products.id = $1",
        &params.to_params(),
    ).await?;
...
pub async fn find_low_stock_products(
    ...
    let rows = client.query(
        "SELECT products.id, products.name, products.stock FROM products WHERE products.stock AND (products.is_active = TRUE) ORDER BY products.stock ASC",
        &params.to_params(),
    ).await?;
