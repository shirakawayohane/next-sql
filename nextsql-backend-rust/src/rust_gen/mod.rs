...
/// Compute enum names referenced by 2 or more modules (should go in shared types.rs).
pub fn compute_shared_enums(modules: &[&Module], schema: &DatabaseSchema) -> HashSet<String> {
    let mut enum_ref_count: HashMap<String, usize> = HashMap::new();
    for module in modules {
        let tables = collect_referenced_tables(module);
        let enums = collect_used_enum_names(&tables, schema);
        for e in enums {
            *enum_ref_count.entry(e).or_insert(0) += 1;
        }
    }
    enum_ref_count.into_iter()
        .filter(|(_, count)| *count >= 2)
        .map(|(name, _)| name)
        .collect()
}

/// Generate a shared types.rs file containing valtype definitions and shared enum definitions.
/// Shared enums are those referenced by multiple modules.
pub fn generate_valtype_file(modules: &[&Module], schema: &DatabaseSchema) -> GeneratedRustFile {
    ...
    // Generate shared enums (referenced by 2+ modules)
    let shared_enums = compute_shared_enums(modules, schema);
    generate_db_enums(&mut out, schema, &shared_enums);
...
/// Includes valtype and enum definitions inline (for single-file use).
pub fn generate_rust_file(module: &Module, schema: &DatabaseSchema) -> GeneratedRustFile {
    generate_rust_file_with_options(module, schema, false, &HashSet::new())
}

/// Generate the full .rs file content from a module and schema.
/// When `skip_valtype_generation` is true, valtype structs are not emitted inline.
/// `shared_enums` contains enum names already in types.rs (excluded from inline generation).
pub fn generate_rust_file_with_options(module: &Module, schema: &DatabaseSchema, skip_valtype_generation: bool, shared_enums: &HashSet<String>) -> GeneratedRustFile {
    generate_rust_file_full(module, schema, skip_valtype_generation, &NamingConfig::default(), shared_enums)
}

/// Generate the full .rs file content with all options including naming config.
pub fn generate_rust_file_full(module: &Module, schema: &DatabaseSchema, skip_valtype_generation: bool, naming: &NamingConfig, shared_enums: &HashSet<String>) -> GeneratedRustFile {
    let mut out = String::new();
    ...
    // Generate Rust enums only for PostgreSQL enum types used by this module but not in shared types.rs
    let referenced_tables = collect_referenced_tables(module);
    let used_enums: HashSet<String> = collect_used_enum_names(&referenced_tables, schema)
        .difference(shared_enums)
        .cloned()
        .collect();
    generate_db_enums(&mut out, schema, &used_enums);
...
/// Collect all table names referenced in the module's queries and mutations.
fn collect_referenced_tables(module: &Module) -> HashSet<String> {
    let mut tables = HashSet::new();
    for toplevel in &module.toplevels {
        match toplevel {
            TopLevel::Query(q) => {
                for stmt in &q.body.statements {
                    match stmt {
                        QueryStatement::Select(s) => {
                            tables.insert(s.from.table.clone());
                            for join in &s.from.joins {
            ...
            TopLevel::Mutation(m) => {
                for item in &m.body.items {
                    match item {
                        MutationBodyItem::Mutation(ms) => match ms {
                            MutationStatement::Insert(i) => { tables.insert(i.into.name.clone()); }
                            MutationStatement::Update(u) => { tables.insert(u.target.name.clone()); }
                            MutationStatement::Delete(d) => { tables.insert(d.target.name.clone()); }
                        },
                    }
                }
            }
...
/// Collect enum type names used by the given tables' columns.
fn collect_used_enum_names(tables: &HashSet<String>, schema: &DatabaseSchema) -> HashSet<String> {
    let mut used_enums = HashSet::new();
    for table_name in tables {
        if let Some(table) = schema.get_table(table_name) {
            for col in &table.columns {
                collect_enum_from_type(&col.column_type, schema, &mut used_enums);
            }
        }
    }
    used_enums
...
fn collect_enum_from_type(typ: &Type, schema: &DatabaseSchema, used: &mut HashSet<String>) {
    match typ {
        Type::UserDefined(name) if schema.enums.contains_key(name) => {
            used.insert(name.clone());
        }
        Type::Optional(inner) | Type::Array(inner) => collect_enum_from_type(inner, schema, used),
        _ => {}
    }
}

/// Generate Rust enum definitions for PostgreSQL enum types used in this module.
fn generate_db_enums(out: &mut String, schema: &DatabaseSchema, used_enums: &HashSet<String>) {
...
"#;
    ...
    fn test_only_used_enums_are_generated() {
        ...
        table.add_column(ColumnSchema {
            name: "role".to_string(),
            column_type: Type::UserDefined("role_type".to_string()),
            nullable: false,
            primary_key: false,
            has_default: false,
            default_value: None,
        ...
        // Also add a table for unused_status that is NOT referenced by the module
        let mut other_table = TableSchema::new("tasks".to_string());
        other_table.add_column(ColumnSchema {
            name: "id".to_string(),
            column_type: Type::BuiltIn(BuiltInType::Uuid),
            nullable: false,
            primary_key: true,
        ...
        other_table.add_column(ColumnSchema {
            name: "status".to_string(),
            column_type: Type::UserDefined("unused_status".to_string()),
            nullable: false,
            primary_key: false,
            has_default: false,
            default_value: None,
        });
        schema.add_table(other_table);

        // Module only queries the accounts table
        let module = Module {
            toplevels: vec![TopLevel::Query(Query {
                decl: QueryDecl {
                    name: "findAllAccounts".to_string(),
                    arguments: vec![],
                },
                body: QueryBody {
                    statements: vec![QueryStatement::Select(SelectStatement {
                        ...
                        clauses: vec![QueryClause::Select(vec![SelectExpression {
                            expr: Expression::Atomic(AtomicExpression::Column(
                                Column::WildcardOf("accounts".to_string(), None),
                            )),
                            alias: None,
                        }])],
                        unions: vec![],
